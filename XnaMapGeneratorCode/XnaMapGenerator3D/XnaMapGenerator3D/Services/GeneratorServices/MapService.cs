...
namespace XnaMapGenerator3D.Services.GeneratorServices
{
    ...
    public class MapService : IMapService
    {
        ...
        public MapService(BGame game , MapGenService mapgenservice, int mapX, int mapY, int mapZ)
        {
        ...
        public void LoadMap(LoadMapParams loadMapParams)
        {
            ...
            BrnVoronoi.Models.VoronoiGraph voronoiMap = null;
            Smooth(loadMapParams);

            //voronoiMap = Fortune.ComputeVoronoiGraph(loadMapParams.Points);
            var mapper = new BrnVoronoi.VoronoiMapper(loadMapParams.Points);
        ...
        private void ImproveMapData(VoronoiGraph voronoiMap, bool fix = false)
        {
            ...
            foreach (VoronoiEdge edge in voronoiMap.Edges)
            {
                ...
                Center cntrLeft = fact.CenterFactory(_game,(float) edge.LeftData[0], 0, (float) edge.LeftData[1]);
                Center cntrRight = fact.CenterFactory(_game, (float) edge.RightData[0], 0, (float) edge.RightData[1]);
            ...
            //foreach (Center c in Program.AppMap.Centers)
            //{
            //    c.FixBorders();
            //    c.SetEdgeAreas();
            //    c.OrderCorners();
