...
using XnaMapGenerator3D.Helpers;
using XnaMapGenerator3D.Models;

namespace XnaMapGenerator3D.Services.GeneratorServices
{
    ...
    public class IslandService : IIslandService
    {
        ...
        public void CreateIsland()
        {
            ...
            foreach (var c in _mapGen.Corners.Values.SelectMany(x => x))
            {
                ...
                //c.Water = (c.Touches.Any(x => x.Water)) ? true : false;
            }

            CalculateElevations();

            //DistributeElevations();
            ...
            foreach (Corner c in _mapGen.Corners.Values.SelectMany(x => x).Where(x => x.Coast))
            {
                ...
                //var firsttang = first.Adjacents.FirstOrDefault(x => x != c && x.Coast);
                //var secondtang = second.Adjacents.FirstOrDefault(x => x != c && x.Coast);

                //var newc = Vector3.CatmullRom(firsttang.Point, first.Point, second.Point, secondtang.Point, 0.5f);
            ...
            {
                //ct.Elevation = (float) (ct.Corners.Sum(x => x.Elevation) / ct.Corners.Count());
                ...
                //ct.Coast = (ct.Corners.Values.SelectMany(x => x).Any(x => x.Coast)) ? true : false;

                ct.Moisture = (float) (ct.Corners.Sum(x => x.Moisture) / ct.Corners.Count());

                //CalculateCenterMoisture(ct);

                ct.SetBiome();
                //ct.Biome = "Ocean";
                //ct._biomeColor = Color.Black;
        ...
        private bool IsLandShape(Vector3 Vector3)
        {
            ...
            var islandRandom = new Random(BGame.RandomSeed);
        ...
        {
            Random rnd = new Random(BGame.RandomSeed);
